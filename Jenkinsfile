	...
	stages {
		stage('Preparacion') { // for display purposes
		      ...
		      steps {
				...
				sh 'ssh root@10.0.11.10 cp -r /var/www/html/ /opt/lastversionweb'
				sh 'ssh root@10.0.11.10 cp -r /var/www/gestion/ /opt/lastversiongestion'
				// eliminar version actual
				sh 'ssh root@10.0.11.10 rm -rf /var/www/html/*'
				sh 'ssh root@10.0.11.10 rm -rf /var/www/gestion/*'
				}		
		   }
		...
		stage('Despliegue') { // en servidor web
			steps {
				...
				sh 'scp -r gestion/* root@10.0.11.10:/var/www/gestion'
				sh 'scp -r webapp/* root@10.0.11.10:/var/www/html'
				}		     
		    }
